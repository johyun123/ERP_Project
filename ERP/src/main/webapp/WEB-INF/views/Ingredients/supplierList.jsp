...
<div class="content">
    ...
    <div class="table-card">
        ...
        <table class="data-table">
            <thead><tr><th>거래처명</th><th>유형</th><th>대표자</th><th>주소</th><th>계약서</th><th>등록일</th><th>관리</th></tr></thead>
            <tbody>
            <c:choose>
                <c:when test="${empty list}"><tr class="empty-row"><td colspan="7">등록된 거래처가 없습니다.</td></tr></c:when>
                <c:otherwise>
                    <c:forEach var="s" items="${list}">
                    <tr>
                        <td><strong>${s.supplier_name}</strong></td>
                        <td><c:if test="${not empty s.supplier_type}"><span class="badge badge-normal">${s.supplier_type}</span></c:if></td>
                        <td>${empty s.ceo_name ? '-' : s.ceo_name}</td>
                        <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${empty s.address ? '-' : s.address}</td>
                        <td>
                            <c:choose>
                                <c:when test="${not empty s.contract_file}">
                                    <button class="btn btn-edit" onclick="viewContract('${s.contract_file}')">조회</button>
                                </c:when>
                                <c:otherwise><span style="color:var(--text-muted);">없음</span></c:otherwise>
                            </c:choose>
                        </td>
                        <td>${s.created_at}</td>
                        <td>
...
<%-- [1순위] 거래처 등록 모달 (supplierRegist.jsp 내용 팝업화) --%>
<div class="modal-overlay" id="registerModal">
    <div class="modal" style="width:560px;max-width:95vw;">
        ...
        <form action="/inventory/vendor/register" method="post" enctype="multipart/form-data">
            ...
            <div class="form-row">
                <div class="form-group"><label>대표자명</label><input type="text" name="ceo_name" placeholder="대표자 이름"></div>
                <div class="form-group"><label>주소</label><input type="text" name="address" placeholder="거래처 주소"></div>
            </div>
            <div class="form-group">
                <label>계약서 첨부</label>
                <input type="file" name="contract_file" accept=".pdf,.doc,.docx,.jpg,.png">
                <small style="color:var(--text-muted);font-size:0.78rem;display:block;margin-top:4px;">PDF, Word, 이미지 파일 업로드 가능</small>
            </div>
            <div class="form-group">
                <label>비고</label>
...
<div class="modal-overlay" id="editModal">
    <div class="modal">
        ...
        <form action="/inventory/vendor/update" method="post">
            ...
            <div class="form-row">
                <div class="form-group"><label>거래처명 *</label><input type="text" name="supplier_name" id="edit_supplier_name" required></div>
                <div class="form-group"><label>거래처 유형</label>
                    <select name="supplier_type" id="edit_supplier_type">
                        <option value="">선택하세요</option>
                        <option value="원두">원두</option><option value="유제품">유제품</option>
                        <option value="시럽/소스">시럽/소스</option><option value="소모품">소모품</option><option value="기타">기타</option>
                    </select>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group"><label>대표자명</label><input type="text" name="ceo_name" id="edit_ceo_name"></div>
                <div class="form-group"><label>주소</label><input type="text" name="address" id="edit_address"></div>
            </div>
            <div class="form-group"><label>비고</label><textarea name="note" id="edit_note" rows="3"></textarea></div>
            <div class="modal-footer">
...
function viewContract(filepath) {
    ...
    if (['jpg','jpeg','png','gif'].includes(ext)) {
        ...
        viewer.innerHTML = '<iframe src="'+filepath+'" width="100%" height="400px" style="border:none;"></iframe>';
    } else {
