...
<body>
	...
	<div class="calendar-page">
		<div class="calendar-header">
			...
			<h3 id="calendarTitle" onclick="showYearSelector()"
...
attendanceNames["<%=row.get("work_date")%>"].push("<%=row.get("name")%>
		...
		<%}
			...
			function renderCalendar() {
				...
				document.getElementById("calendarTitle").innerText = currentYear
						+ "년 " + (currentMonth + 1) + "월";
				...
				for (let day = 1; day <= lastDate; day++) {
					...
					row += "<td onclick=\"moveDate('" + date + "')\">"
							...
							+ countText + "</div>" + "<div class='tooltip'>"
							+ nameText + "</div>" + "</td>";
			...
			function showYearSelector() {
				...
				for (let y = startYear; y <= endYear; y++) {
					html += "<div style='padding:8px; cursor:pointer;' onclick='selectYear("
							+ y + ")'>" + y + "년</div>";
				}
			...
			function selectYear(year) {
				...
				for (let i = 0; i < 3; i++) {
					...
					for (let j = 1; j <= 4; j++) {
						let m = i * 4 + j;
						html += "<td onclick='selectMonth(" + m + ")'>" + m
								+ "월</td>";
					}
					html += "</tr>";
				}
