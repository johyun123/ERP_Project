...
public interface HRMMapper {
	...
	/* ===== [추가] 페이징 조회 ===== */
	List<Employees> searchEmployeesWithPaging(@Param("name") String name, @Param("position") String position,
			@Param("is_active") Integer is_active, @Param("offset") int offset, @Param("size") int size);

	/* ===== [추가] 전체 개수 ===== */
	int countEmployees(@Param("name") String name, @Param("position") String position,
			@Param("is_active") Integer is_active);
