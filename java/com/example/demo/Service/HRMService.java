...
public class HRMService {
	...
	/**
	 * 직원 삭제 (FK 순서 고려) users.id → employees.id (FK) attendances.employee_id →
	 * employees.id (FK)
	 *
	 * 삭제 순서: 1. employees.id 조회 2. users 삭제 (employees 참조) 3. attendances 삭제
	 * (employees 참조) 4. employees 삭제
	 */
	...
	/* ===== [추가] 페이징 직원 조회 ===== */
	public List<Employees> searchEmployeesWithPaging(String name, String position, Integer isActive, int offset,
			int size) {
